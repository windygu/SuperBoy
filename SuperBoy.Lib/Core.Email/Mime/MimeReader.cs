    ...
    public class MimeReader
    {
        ...
        private int ParseHeaders()
        {
            ...
            while (_lines.Count > 0 && !string.IsNullOrEmpty(_lines.Peek()))
            {
                ...
                var headerValue = line.Substring(separatorIndex + 1).Trim(HeaderWhitespaceChars);
        ...
        {
            foreach (var key in _entity.Headers.AllKeys)
            {
                switch (key)
                {
        ...
        public static string GetMediaSubType(string mediaType)
        {
            ...
            else
            {
                ...
                {
                    var mainType = GetMediaMainType(mediaType);
