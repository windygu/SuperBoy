...
namespace EmailHelp
{
    public class MailPoper
    {
        ...
        {
            var ps = PopConfig.Create().PopSetting;
        ...
        {
            using (var client = new Pop3Client(hostname, port, useSsl, username, password))
            {
                client.Trace += new Action<string>(Console.WriteLine);
                client.Authenticate();
                client.Stat();
                var maillist = new List<MailMessageEx>();
    ...
    public class PopConfig
    {
        ...
        private string ConfigFile
        {
            get
            {
                var configPath = ConfigurationManager.AppSettings["PopConfigPath"];
