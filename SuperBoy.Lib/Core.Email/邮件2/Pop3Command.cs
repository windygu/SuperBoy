...
namespace EmailHelp
{
    ...
    internal abstract class Pop3Command<T> : IDisposable where T : Pop3Response
    {
        ...
        private string WriteReceivedBytesToBuffer(int bytesReceived)
        {
            ...
            var contents = _responseContents.ToArray();
        ...
        {
            var bytesReceived = _networkStream.EndRead(ar);
            var message = WriteReceivedBytesToBuffer(bytesReceived);
        ...
        {
            var message = string.Join(string.Empty, args);
            OnTrace(message);
        ...
        {
            var position = header.Length + 2;
            var stream = new MemoryStream(bytes, position, bytes.Length - position);
            return stream;
        }
