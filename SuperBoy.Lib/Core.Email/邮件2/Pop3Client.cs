...
namespace EmailHelp
{
    ...
    public sealed class Pop3Client : IDisposable
    {
        ...
        private void Connect()
        {
            ...
            using (var command = new ConnectCommand(_client, _hostname, _port, _useSsl))
            {
        ...
        public void Authenticate()
        {
            ...
            using (var userCommand = new UserCommand(_clientStream, _username))
            {
                ExecuteCommand<Pop3Response, UserCommand>(userCommand);
            }

            //execute the pass command.
            using (var passCommand = new PassCommand(_clientStream, _password))
            {
        ...
        public void Dele(Pop3ListItem item)
        {
            ...
            using (var command = new DeleCommand(_clientStream, item.MessageId))
            {
        ...
        public void Noop()
        {
            using (var command = new NoopCommand(_clientStream))
            {
        ...
        public void Rset()
        {
            using (var command = new RsetCommand(_clientStream))
            {
        ...
        public Stat Stat()
        {
            ...
            using (var command = new StatCommand(_clientStream))
            {
        ...
        public List<Pop3ListItem> List()
        {
            ...
            using (var command = new ListCommand(_clientStream))
            {
        ...
        {
            ListResponse response;
            using (var command = new ListCommand(_clientStream, messageId))
            {
                response = ExecuteCommand<ListResponse, ListCommand>(command);
            }
        ...
        public MimeEntity RetrMimeEntity(Pop3ListItem item)
        {
            ...
            using (var command = new RetrCommand(_clientStream, item.MessageId))
            {
                response = ExecuteCommand<RetrResponse, RetrCommand>(command);
            }

            var reader = new MimeReader(response.MessageLines);
        ...
        public MailMessageEx Top(int messageId, int lineCount)
        {
            ...
            RetrResponse response;
            using (var command = new TopCommand(_clientStream, messageId, lineCount))
            {
                response = ExecuteCommand<RetrResponse, TopCommand>(command);
            }

            var reader = new MimeReader(response.MessageLines);
            var entity = reader.CreateMimeEntity();
        ...
        public MailMessageEx RetrMailMessageEx(Pop3ListItem item)
        {
            ...
            var message = RetrMimeEntity(item).ToMailMessageEx();
            if (message != null)
            {
        ...
        public void Quit()
        {
            using (var command = new QuitCommand(_clientStream))
            {
            ...
            where TCommand : Pop3Command<TResponse>
        {
            ...
            var response = (TResponse)command.Execute(CurrentState);
            EnsureResponse(response);
            return response;
        }
