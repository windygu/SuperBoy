...
{ 
    ...
    public class XmlHelper //: IDisposable 
    {
        ...
        {
            var xml = XmlConvertor.ObjectToXml(obj, true);
        ...
        { 
            ...
            docXml.Load(@XmlPath); 
            var xn = docXml.GetElementsByTagName(node); 
        ...
        { 
            var objXmlDoc = new XmlDocument(); 
            objXmlDoc.Load(xmlPath); 
            var objRootNode = objXmlDoc.SelectSingleNode(mailNode); 
        ...
        { 
            var MainNode = _xmlDoc.SelectSingleNode(mainNode); 
            var objElem = _xmlDoc.CreateElement(node);             
   ...
   *      <description>描述内容 </description> 
      *    </page> 
        ...
        { 
            ...
            var docXml = new XmlDocument(); 
            docXml.Load(@XmlPath); 
            var xn = docXml.DocumentElement.ChildNodes; 
            //遍历第一层节点 
            ...
            { 
                ...
                { 
                    ...
                    { 
                        var xx = element.ChildNodes; 
                        if (xx.Count > 0) 
                        { 
                            for (var i = 0; i < arrayLength & i < xx.Count; i++) 
                            { 
