...
namespace Core.XML
{
    public class XmlProcess
    {
        ...
        private static XmlDocument XMLLoad(string strPath)
        {
            ...
            try
            {
                var filename = AppDomain.CurrentDomain.BaseDirectory.ToString() + strPath;
                if (File.Exists(filename)) xmldoc.Load(filename);
            }
            catch (Exception e)
            { }
        ...
        public static bool WriteXmlByDataSet(string strXmlPath, string[] columns, string[] columnValue)
        {
            try
            {
                ...
                var strXsdPath = strXmlPath.Substring(0, strXmlPath.IndexOf(".")) + ".xsd";
                var ds = new DataSet();
                ds.ReadXmlSchema(GetXmlFullPath(strXsdPath)); //读XML架构，关系到列的数据类型
                ds.ReadXml(GetXmlFullPath(strXmlPath));
                var dt = ds.Tables[0];
                var newRow = dt.NewRow();                 //在原来的表格基础上创建新行
                for (var i = 0; i < columns.Length; i++)      //循环给一行中的各个列赋值
                {
