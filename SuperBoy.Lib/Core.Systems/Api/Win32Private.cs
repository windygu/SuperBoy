...
namespace DotNet.Utilities
{
    ...
    public static partial class Win32
    {
        ...
        private enum DriverError : byte
        {
            ...
            /// 设备无错误。
            /// </summary>
            SmartNoError = 0, // No error
            /// <summary>
            /// 设备IDE控制器错误。
            /// </summary>
            SmartIdeError = 1, // Error from IDE controller
            /// <summary>
            /// 无效的命令标记。
            /// </summary>
            SmartInvalidFlag = 2, // Invalid command flag
            /// <summary>
            /// 无效的命令数据。
            /// </summary>
            SmartInvalidCommand = 3, // Invalid command byte
            /// <summary>
            /// 缓冲区无效（如缓冲区为空或地址错误）。
            /// </summary>
            SmartInvalidBuffer = 4, // Bad buffer (null, invalid addr..)
            /// <summary>
            /// 设备编号错误。
            /// </summary>
            SmartInvalidDrive = 5, // Drive number not valid
            /// <summary>
            /// IOCTL错误。
            /// </summary>
            SmartInvalidIoctl = 6, // Invalid IOCTL
            /// <summary>
            /// 无法锁定用户的缓冲区。
            /// </summary>
            SmartErrorNoMem = 7, // Could not lock user's buffer
            /// <summary>
            /// 无效的IDE注册命令。
            /// </summary>
            SmartInvalidRegister = 8, // Some IDE Register not valid
            /// <summary>
            /// 无效的命令设置。
            /// </summary>
            SmartNotSupported = 9, // Invalid cmd flag set
            /// <summary>
            /// 指定要查找的设别索引号无效。
            /// </summary>
            SmartNoIdeDevice = 10
        ...
        private static HDiskInfo GetHddInfoNt(byte driveIndex)
        {
            ...
            // 获取设备的句柄。
            var hDevice =
                CreateFile(string.Format(@"\\.\PhysicalDrive{0}", driveIndex), GenericRead | GenericWrite,
            ...
            if(0 == DeviceIoControl(hDevice, SmartGetVersion, IntPtr.Zero, 0, ref vers,
                (uint)Marshal.SizeOf(vers),
                ref bytesReturned,
                IntPtr.Zero))
            {
                CloseHandle(hDevice);
                throw new IOException(string.Format(ResourcesApi.Win32DeviceIoControlErr, "SMART_GET_VERSION"));
            }
            // 检测IDE控制命令是否支持
            if(0 == (vers.fCapabilities & 1))
            {
                CloseHandle(hDevice);
                throw new IOException(ResourcesApi.Win32DeviceIoControlNotSupport);
            }
                ...
                IntPtr.Zero))
            {
                ...
                throw new IOException(
                        string.Format(ResourcesApi.Win32DeviceIoControlErr, "SMART_RCV_DRIVE_DATA"));
            }
            CloseHandle(hDevice);
        ...
        private static HDiskInfo GetHddInfo9X(byte driveIndex)
        {
            ...
            var outParam = new SendCmdOutParams();
            uint bytesReturned = 0;
            var hDevice = CreateFile(@"\\.\Smartvsd", 0, 0, IntPtr.Zero, CreateNew, 0, IntPtr.Zero);
            if(hDevice == IntPtr.Zero)
                throw new UnauthorizedAccessException("打开 smartvsd.vxd 文件失败。");
            if(0 == DeviceIoControl(hDevice, SmartGetVersion, 
                IntPtr.Zero, 0, 
            ...
            inParam.irDriveRegs.bSectorCountReg = 1;
            inParam.irDriveRegs.bSectorNumberReg = 1;
            inParam.cBufferSize = BufferSize;
            if(0 == DeviceIoControl(hDevice, SmartRcvDriveData, ref inParam, (uint)Marshal.SizeOf(inParam), ref outParam, (uint)Marshal.SizeOf(outParam), ref bytesReturned, IntPtr.Zero))
            {
                CloseHandle(hDevice);
                throw new IOException(string.Format(ResourcesApi.Win32DeviceIoControlErr, "SMART_RCV_DRIVE_DATA"));
            }
            // 关闭文件句柄
            CloseHandle(hDevice);
            ChangeByteOrder(outParam.bBuffer.sModelNumber);
        ...
        /// <remarks>
        /// 如果在 <paramref name="uFlags"/> 中使用了<see cref="MfByposition"/> ，则在 <paramref name="uPosition"/> 参数表示菜单项的索引；
        ...
        private const uint SmartGetVersion = 0x00074080; // SMART_GET_VERSION
        private const uint SmartSendDriveCommand = 0x0007c084; // SMART_SEND_DRIVE_COMMAND
        private const uint SmartRcvDriveData = 0x0007c088; // SMART_RCV_DRIVE_DATA
