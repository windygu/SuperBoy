...
namespace Core.Systems
{
    sealed class AdHelper
    {
        ...
        /// homeMTA
        /// </summary>
        private static string _homeMta = ""; //请填写自己的环境变量
        /// <summary>
        /// homeMDB
        /// </summary>
        private static string _homeMdb = ""; //请填写自己的环境变量
        /// <summary>
        /// msExchHomeServerName
        /// </summary>
        private static string _msExchHomeServerName = ""; //请填写自己的环境变量

        ///<summary>
        /// 域名
        ///</summary>
        //private static string DomainName = "td-tech.net";  //实际
        private static string _domainName = "contoso.com";    //测试用
        /// <summary>
        /// LDAP 地址
        /// </summary>
        private static string _ldapDomain = "DC=net,DC=TD-TECH";
        /// <summary>
        /// LDAP绑定路径
        /// </summary>
           private static string _adPath = "LDAP://Contoso.com";     //测试用
          
        private static string _sPrincpleNameTail = "@cinf.com";
        /// <summary>
        /// 登录帐号
        /// </summary>
        private static string _adUser = @"contoso\oa";
        /// <summary>
        /// 登录密码
        /// </summary>
        //private static string ADPassword = "3edc5tgB"; //实际
        private static string _adPassword = "1qaz2wsX";   //测试用
        ...
        public enum LoginResult
        {
            ...
            /// 用户帐号被禁用
            /// 
            LoginUserAccountInactive,
            /// 
            /// 用户密码不正确
            /// 
            LoginUserPasswordIncorrect
        }
        /// <summary>
        /// 用户属性定义标志
        /// </summary>
        public enum AdsUserFlagEnum
        {
            /// 
            ...
            /// 用户帐号禁用标志
            /// 
            AdsUfAccountdisable = 0X0002,
            /// 
            /// 主文件夹标志
            /// 
            AdsUfHomedirRequired = 0X0008,
            /// 
            /// 过期标志
            /// 
            AdsUfLockout = 0X0010,
            ...
            /// 用户密码不是必须的
            /// 
            AdsUfPasswdNotreqd = 0X0020,
            /// 
            /// 密码不能更改标志
            /// 
            AdsUfPasswdCantChange = 0X0040,
            /// 
            /// 使用可逆的加密保存密码
            /// 
            AdsUfEncryptedTextPasswordAllowed = 0X0080,
            /// 
            /// 本地帐号标志
            /// 
            AdsUfTempDuplicateAccount = 0X0100,
            /// 
            /// 普通用户的默认帐号类型
            /// 
            AdsUfNormalAccount = 0X0200,
            /// 
            /// 跨域的信任帐号标志
            /// 
            AdsUfInterdomainTrustAccount = 0X0800,
            /// 
            /// 工作站信任帐号标志
            /// 
            AdsUfWorkstationTrustAccount = 0x1000,
            /// 
            /// 服务器信任帐号标志
            /// 
            AdsUfServerTrustAccount = 0X2000,
            /// 
            /// 密码永不过期标志
            /// 
            AdsUfDontExpirePasswd = 0X10000,
            /// 
            /// MNS 帐号标志
            /// 
            AdsUfMnsLogonAccount = 0X20000,
            /// 
            /// 交互式登录必须使用智能卡
            /// 
            AdsUfSmartcardRequired = 0X40000,
            /// 
            /// 当设置该标志时，服务帐号（用户或计算机帐号）将通过 Kerberos 委托信任
            /// 
            AdsUfTrustedForDelegation = 0X80000,
            /// 
            /// 当设置该标志时，即使服务帐号是通过 Kerberos 委托信任的，敏感帐号不能被委托
            /// 
            AdsUfNotDelegated = 0X100000,
            /// 
            /// 此帐号需要 DES 加密类型
            /// 
            AdsUfUseDesKeyOnly = 0X200000,
            /// 
            /// 不要进行 Kerberos 预身份验证
            /// 
            AdsUfDontRequirePreauth = 0X4000000,
            /// 
            /// 用户密码过期标志
            /// 
            AdsUfPasswordExpired = 0X800000,
            /// 
            /// 用户帐号可委托标志
            /// 
            AdsUfTrustedToAuthenticateForDelegation = 0X1000000
        ...
        {
            _adPath = sAdPath;
            _domainName = sDomainName;
            _adUser = sAdUser;
            _adPassword = sAdUserPwd;
        }
        #endregion
        ...
        {
            var entry = new DirectoryEntry(_adPath, _adUser, _adPassword, AuthenticationTypes.Secure);
            return entry;
        }
        ...
        {
            var entry = new DirectoryEntry(_adPath,
        ...
        {
            var entry = new DirectoryEntry(_adPath + domainReference, _adUser, _adPassword,
        ...
        {
            var entry = new DirectoryEntry(_adPath + domainReference,
        ...
        {
            var de = GetDirectoryObject(commonName, password);
            var deSearch = new DirectorySearcher(de);
            deSearch.Filter = "(&(&(objectCategory=person)(objectClass=user))(cn=" + commonName + "))";
            deSearch.SearchScope = SearchScope.Subtree;
            try
        ...
        {
            var de = GetDirectoryObject(_adUser, _adPassword);
            var deSearch = new DirectorySearcher(de);
            ...
            {
                var result = deSearch.FindOne();
                de = new DirectoryEntry(result.Path, _adUser, _adPassword);
                return de;
            }
            catch(Exception ex)
            {
        ...
        {
            var de = GetDirectoryEntryByAccount(sAmAccountName);
            if (de != null)
            {
                var commonName = de.Properties["cn"][0].ToString();
            ...
            )
        {
            ...
            var samAccountName = account;
            try
            {
                entry = new DirectoryEntry(GetOrganizeNamePath(organizeName), _adUser,
                _adPassword, AuthenticationTypes.Secure);

                user = entry.Children.Add("CN=" + commonName, "user");
                user.Properties["userPrincipalName"].Value = samAccountName + _sPrincpleNameTail;
                ...
                user.Properties["userAccountControl"].Value = //0x200; //ADS_UF_NORMAL_ACCOUNT
                     AdHelper.AdsUserFlagEnum.AdsUfNormalAccount | AdHelper.AdsUserFlagEnum.AdsUfDontExpirePasswd;
                user.CommitChanges();
                if (createMail)
                {
                    user.Invoke("CreateMailbox", new object[] { _homeMdb });
                    user.CommitChanges();
                }
            }
            catch (Exception e)
        ...
        public static void DisableUser(DirectoryEntry de)
        {
            ...
            de.Properties["userAccountControl"][0] =
                AdHelper.AdsUserFlagEnum.AdsUfNormalAccount | AdHelper.AdsUserFlagEnum.AdsUfDontExpirePasswd | AdHelper.AdsUserFlagEnum.AdsUfAccountdisable;
            de.CommitChanges();
        ...
        {
            // to-do: 需要解决密码策略问题
            var oUser = GetDirectoryEntry(commonName);
        ...
        public static bool ChangeAccPassword(string sAMacc, string oldPassword, string newPassword)
        {
            try
            {
                ...
                {
                    // to-do: 需要解决密码策略问题
                    var oUser = GetDirectoryEntryByAccount(sAMacc);
                    //oUser.Invoke("ChangePassword", new Object[] { oldPassword, newPassword });
        ...
        /// <returns></returns>
        public static string MoveUser(string userPath, string targetPath)
        {
            var u = new DirectoryEntry(userPath, _adUser, _adPassword);
            var t = new DirectoryEntry(targetPath, _adUser, _adPassword);
            u.MoveTo(t);
            return u.Path;
        }
        ...
        public static bool RenameUser(string sAcc, string newDisplayName)
        {
            try
            {
                if (IsAccExists(sAcc))
                {
                    var userEntry = GetDirectoryEntryByAccount(sAcc);
            ...
            try
            {
                if (IsAccExists(oldAcc))
                {
                    ...
                    if (IsAccExists(newAcc))
                        return false;
                    var userEntry = GetDirectoryEntryByAccount(oldAcc);
        ...
        {
            adminName = _adUser;
            adminPassword = _adPassword;
            try
            {
                var userEntry = FindObject(adminName, adminPassword, "user", userName);
        ...
        public static bool DeleteADAccount(string account)
        {
            ...
            if (IsAccExists(account))
            {
                var accPath = GetAccPath(account);
                DetTree(accPath);
                return true;
            }
            else
        ...
        public static bool SetAdAccountLocked( string accountName)
        {
            ...
            try
            {
                ...
                user = FindObject(_adUser, _adPassword, "user", accountName);
            ...
            string adminPassword, string category, string name)
        {
            ...
            //{
            de = new DirectoryEntry(_adPath, adminName, adminPassword, AuthenticationTypes.Secure);
            //}
            //else
            //{
            ...
            var queryFilter = string.Format("(&(objectCategory=" +
        ...
        public static bool RenameOu(string oldOuName, string newOuName)
        {
            ...
            try
            {
                ...
                {
                    var userEntry = new DirectoryEntry(GetOrganizeNamePath(oldOuName), _adUser, _adPassword);
        ...
        public static bool CheckOu(string sOu)
        {
            ...
            try
            {
                objOu = new DirectoryEntry(GetOrganizeNamePath(sOu), _adUser, _adPassword);
                var ouName = objOu.Name;
                //if (OUName == sOU)
                objOu.Close();
                objOu = null;
                return true;
            ...
            string adminPassword, string name, string parentOrganizeUnit)
        {
            ...
            var organizeEntry = parentEntry.Children.Add("OU=" +
        ...
        public static bool DeleteOU(string ouName, bool boolComplete)
        {
            try
            {
                ...
                if (CheckOu(ouName))
                {
                    var oUpath = GetOrganizeNamePath(ouName);
                    if (oUpath == "")
                        return false;
                    else
                    {
        ...
        public static string DeleteOU(string ouName)
        {
            try
            {
                if (CheckOu(ouName))
                {
                    var objOu = new DirectoryEntry(GetOrganizeNamePath(ouName));
                    var parOu = objOu.Parent;
        ...
        {
            var de = GetDirectoryEntry(commonName);
            ...
            if (de != null)
            {
                ...
                if (!IsAccountActive(userAccountControl))
                    ...
                    return LoginResult.LoginUserAccountInactive;
                if (GetDirectoryEntry(commonName, password) != null)
                    return LoginResult.LoginUserOk;
                else
                    return LoginResult.LoginUserPasswordIncorrect;
            }
            else
            {
                return LoginResult.LoginUserDoesntExist;
            }
        }
        ...
        {
            var userAccountControlDisabled =
                Convert.ToInt32(AdsUserFlagEnum.AdsUfAccountdisable);
            var flagExists = userAccountControl & userAccountControlDisabled;
            if (flagExists > 0)
                return false;
            else
                return true;
        ...
        public static void DetTree(string path)
        {
            try
            {
                var ent = new DirectoryEntry(path, _adUser, _adPassword);
                ent.DeleteTree();
        ...
        {
            var sb = new StringBuilder();
            sb.Append(_adPath);//LDAP_IDENTITY
            sb.Append("/");
        ...
        public static string SplitOrganizeNameToDn(string organizeName)
        {
            ...
            {
                var allOu = organizeName.Split(new char[] { '/', '\\' });
                for (var i = allOu.Length - 1; i >= 0; i--)
                {
                    var ou = allOu[i];
                    if (sb.Length > 0)
                    {
