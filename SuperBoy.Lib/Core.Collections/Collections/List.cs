...
namespace Core.Collections
{
    ...
    public class List<T> : System.Collections.Generic.List<T>, ICloneable, ICloneable<List<T>>
    {
        ...
        /// <typeparam name="TV">The type of elements in the list.</typeparam>
        [HostProtection(SecurityAction.LinkDemand, Synchronization = true)]
        public static List<TV> Synchronized<TV>(List<TV> list)
        {
            if (list == null)
            {
        ...
        /// <typeparam name="TV">The type of elements in the list.</typeparam>
        public static List<TV> Repeat<TV>(TV value, int count)
        {
            if (count < 0)
            {
