...
namespace Core.Zip
{
    ...
    public class GZipUtil
    {
        ...
        public static T GZip<T>(Stream stream, CompressionMode mode) where T : Stream
        {
            ...
            using (Stream sg = new GZipStream(stream, mode))
            {
                while (true)
                {
                    ...
                    var size = sg.Read(writeData, 0, writeData.Length);
                    if (size > 0)
                    {
        ...
        {
            using (var stream = GZip<MemoryStream>(new MemoryStream(bytData), CompressionMode.Compress))
            {
        ...
        {
            using (var stream = GZip<MemoryStream>(new MemoryStream(bytData), CompressionMode.Decompress))
            {
                return stream.ToArray();
            }
        }
        ...
        {
            var byteTempArray = ObjectToByteArray(obj);
        ...
        {
            var byteTempArray = GZipUtil.Decompress(byteArray);
        ...
        public static void DecompressFile(string sourceFile, string destinationFile)
        {
            ...
            try
            {
                ...
                buffer = new byte[4];
                var num = ((int)stream.Length) - 4;
                stream.Position = num;
                stream.Read(buffer, 0, 4);
                stream.Position = 0L;
                var buffer2 = new byte[BitConverter.ToInt32(buffer, 0) + 100];
                var offset = 0;
                var count = 0;
                while (true)
                {
                    var num5 = stream3.Read(buffer2, offset, 100);
                    if (num5 == 0)
                    {
                        break;
                    }
