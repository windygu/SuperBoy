...
public class LzwDecoder
{
    ...
    {
        var nullCode = -1;
        var pixelCount = width * height;//获取原图像的像素数，公式为 像素数 = 图像长度*图像高度
        var pixels = new byte[pixelCount];
        var dataSize = Read();          //图像编码流的第一个字节(byte)存放的是数据位大小，在gif通常为1,4,8
        var codeSize = dataSize + 1;    //编码位大小，根据lzw算法的要求，编码位的大小 = 数据位大小+1 ，针对gif，有如下对应关系 1->3 4->5 ->9,而最大的codeSize为12
        var clearFlag = 1 << dataSize;  //在lzw算法有两个特殊标记，clearFlag为其中的清除标记，此后的编码将重头再来，这样做可以防止编码位无限增大
        var endFlag = clearFlag + 1;    //lzw算法两个特殊标记之一，endFlag为结束标记，表示一次编码的结束  endFlag=clearFlag+1
        var available = endFlag + 1;    //初始的可用编码大小，因为0-(clear-1)为元数据，所以均可用，不必研究，此处从能形成压缩的编码开始算起

        var code = nullCode;     //用于存储当前的编码值
        var oldCode = nullCode; //用于存储上一次的编码值
        var codeMask = (1 << codeSize) - 1;//表示编码的最大值，如果codeSize=5,则code_mask=31
        var bits = 0;//在编码流中数据的保存形式为byte，而实际编码过程中是找实际编码位来存储的，比如当codeSize=5的时候，那么实际上5bit的数据就应该可以表示一个编码，这样取出来的1个字节就富余了3个bit，这3个bit用于和第二个字节的后两个bit进行组合，再次形成编码值，如此类推

        var prefix = new int[MaxStackSize];          //用于保存前缀的集合
        var suffix = new int[MaxStackSize];          //用于保存后缀
        var pixelStatck = new int[MaxStackSize + 1]; //用于临时保存数据流

        var top = 0;
        var count = 0; //在下面的循环中，每次会获取一定量的编码的字节数组，而处理这些数组的时候需要1个个字节来处理，count就是表示还要处理的字节数目
        var bi = 0;    //count表示还剩多少字节需要处理，而bi则表示本次已经处理的个数
        var i = 0;     //i代表当前处理得到像素数

        var data = 0;          //表示当前处理的数据的值
        var first = 0;         //一个字符串重的第一个字节
        var inCode = nullCode; //在lzw中，如果认识了一个编码所代表的数据entry，则将编码作为下一次的prefix，此处inCode代表传递给下一次作为前缀的编码值
        ...
        while (i < pixelCount)
        {
            ...
            if (top == 0)
            {
                ...
                code = data & codeMask; //获取data数据的编码位大小bit的数据
