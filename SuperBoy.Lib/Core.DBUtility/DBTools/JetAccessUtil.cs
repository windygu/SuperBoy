    ...
    public class JetAccessUtil
    {
        ...
        public static string CreateMDB(string mdbFilePath, string password)
        {
            try
            {
                ...
                var objCatalog = Activator.CreateInstance(Type.GetTypeFromProgID("ADOX.Catalog"));
                var oParams = new object[] { connStr };
        ...
        {
            var connStr = "Provider=Microsoft.Jet.OLEDB.4.0;Jet OLEDB:Engine Type=5;";
            var connStrTemp = connStr;
            var tmpPath = mdbFilePath + ".tmp";
            if (password == null || password.Trim() == "")
            {
                connStr += "Data Source=" + mdbFilePath;
            ...
            try
            {
                var objJro = Activator.CreateInstance(Type.GetTypeFromProgID("JRO.JetEngine"));
                var oParams = new object[] { connStr, connStrTemp };
        ...
        {
            var connStr = string.Concat("Provider=Microsoft.Jet.OLEDB.4.0;",
            ...
            using (var conn = new OleDbConnection(connStr))
            {
                try
                {
                    ...
                    var sqlOldPwd = (oldPwd == null || oldPwd.Trim() == "") ? "null" : "[" + oldPwd + "]";
                    var sqlNewPwd = (newPwd == null || newPwd.Trim() == "") ? "null" : "[" + newPwd + "]";
                    var cmd = new OleDbCommand(string.Concat("ALTER DATABASE PASSWORD ", sqlNewPwd, " ", sqlOldPwd),
                           conn);
                    cmd.ExecuteNonQuery();
                    conn.Close();
        ...
        public static Dictionary<string, string> ListColumns(string mdbFilePath, string password, string tableName)
        {
            ...
            {
                conn.Open();
                var schemaTable = GetReaderSchema(tableName, conn);
