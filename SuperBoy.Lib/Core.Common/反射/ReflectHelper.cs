...
namespace Core.Common
{
    ...
    public static class ReflectHelper
    {
        ...
        public static void FillInstanceValue(object model, DataRow dRow)
        {
            ...
            for (var i = 0; i < dRow.Table.Columns.Count; i++)
            {
                var property = type.GetProperty(dRow.Table.Columns[i].ColumnName);
                if (property != null)
                {
        ...
        public static void FillInstanceValue(object model, IDataReader dr)
        {
            ...
            {
                var property = type.GetProperty(dr.GetName(i));
                if (property != null)
                {
        ...
        public static object InvokeMethodOrGetProperty(Type refType, string funName, object[] objInitial, params object[] funParams)
        {
            ...
            else
            {
                ...
                if (targetMethod != null)
                {
                    ...
                    try
                    {
                        var objInstance = System.Activator.CreateInstance(refType, objInitial);
        ...
        public static string GetManifestString(Type assemblyType, string charset, string resName)
        {
            ...
            var st = asm.GetManifestResourceStream(string.Concat(assemblyType.Namespace,
                ".", resName.Replace("/", ".")));
            if (st == null) { return ""; }
