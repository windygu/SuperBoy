...
namespace Core.Common
{
    ...
    public class FormulaCalculator
    {
        ...
        /// 最高运算级别常量
        /// </summary>
        protected const int MaxLevel = 99;
        ...
        private static int GetOperatorLevel(string o)
        {
            for (var i = 0; i < _level.Length; i++)
                if ((string)_level[i][0] == o)
                    return (int)_level[i][1];
            return -1;
        }
        ...
        {
            for (var i = 0; i < _level.Length; i++)
                if (v.StartsWith((string)_level[i][0]))
                    return (string)_level[i][0];
            return null;
        }

        #region 运算符与支持的函数
        private static object[][] _level = new object[][]
            {
                new object[]{",",0},
                new object[]{"=",1},
                new object[]{">=",1},
                ...
                //数学函数
                new object[]{"ROUND(",MaxLevel},
                new object[]{"TRUNC(",MaxLevel},
                new object[]{"MAX(",MaxLevel},
                new object[]{"MIN(",MaxLevel},
                new object[]{"ABS(",MaxLevel},
                new object[]{"SUM(",MaxLevel},
                new object[]{"AVERAGE(",MaxLevel},
                new object[]{"SQRT(",MaxLevel},
                new object[]{"EXP(",MaxLevel},
                new object[]{"LOG(",MaxLevel},
                new object[]{"LOG10(",MaxLevel},

                //三角函数
                new object[]{"SIN(",MaxLevel},
                new object[]{"COS(",MaxLevel},
                new object[]{"TAN(",MaxLevel},

                //条件函数
                new object[]{"IF(",MaxLevel},
                new object[]{"NOT(",MaxLevel},
                new object[]{"AND(",MaxLevel},
                new object[]{"OR(",MaxLevel},
    };
        ...
        {
            var calc = new FormulaCalculator(expression, dataProvider);
        ...
        public decimal[] Calculate()
        {
            ...
            if (GetOperatorLevel(_opt) != MaxLevel)
            {
        ...
        {
            //查找对应的右括号
            var inx = GetIndex(_expression);
            if (inx == -1)
                throw new Exception("缺少\")\"");

            var l = _expression.Substring(0, inx);
        ...
        private decimal[] CalculateTwoParms()
        {
            ...
            else
            {
                ...
                {
                    ex += op;
                    if (GetOperatorLevel(op) == MaxLevel)
                    {
                        var pos = GetIndex(right);
                ...
                var calc = new FormulaCalculator(ex, _data);
                var rl = calc.Calculate();
        ...
        {
            var calc = new FormulaCalculator(expression, _data);
            var values = calc.Calculate();
