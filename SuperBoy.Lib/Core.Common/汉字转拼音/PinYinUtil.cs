...
namespace Core.Common
{
    ...
    public class PinYinUtil
    {
        ...
        /// 包含字符 ASC 码的整形数组。
        /// </SUMMARY>
        private static int[] _pinyinValues = new int[]
        {
        ...
        /// 包含汉字拼音的字符串数组。
        /// </SUMMARY>
        private static string[] _pinyinNames = new string[]
        {
        ...
        public static string CHSToPinyin(string chs, string separator, bool initialCap)
        {
            ...
            for (var j = 0; j < nowchar.Length; j++)
            {
                ...
                else
                {
                    ...
                    for (var i = (_pinyinValues.Length - 1); i >= 0; i--)
                    {
        ...
        public static string GetFirstPy(string chsStr)
        {
            ...
            {
                var s = chsStr.Substring(0, 1).ToUpper();
                    ...
                    else
                    {
                        switch (asc)
                        {
                            ...
                            default:
                                for (var i = (_pinyinValues.Length - 1); i >= 0; i--)
                                {
                                    if (_pinyinValues[i] <= asc) //判断汉字的拼音区编码是否在指定范围内
                                    {
                                        pystr += _pinyinNames[i];//如果不超出范围则获取对应的拼音
                                        break;
                                    }
                                }
                                break;
