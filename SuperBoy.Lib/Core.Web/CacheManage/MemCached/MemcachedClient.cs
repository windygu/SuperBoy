...
namespace Core.Web.CacheManage.Memcached
{
	...
	public class MemcachedClient {
		...
		private static Dictionary<string, MemcachedClient> _instances = new Dictionary<string, MemcachedClient>();
		private static LogAdapter _logger = LogAdapter.GetLogger(typeof(MemcachedClient));
		...
		public static void Setup(string name, string[] servers) {
			...
			_instances[name] = new MemcachedClient(name, servers);
		}
		...
		/// </summary>
		private static MemcachedClient _defaultInstance = null;
		public static MemcachedClient GetInstance() {
			return _defaultInstance ?? (_defaultInstance = GetInstance("default"));
		}
		...
		public static MemcachedClient GetInstance(string name) {
			MemcachedClient c;
			if (_instances.TryGetValue(name, out c)) {
				return c;
			} else {
				var config = ConfigurationManager.GetSection("beitmemcached") as NameValueCollection;
		...
		/// </summary>
		public int SendReceiveTimeout { get { return _serverPool.SendReceiveTimeout; } set { _serverPool.SendReceiveTimeout = value; } }
		...
		/// </summary>
		public int ConnectTimeout { get { return _serverPool.ConnectTimeout; } set { _serverPool.ConnectTimeout = value; } }
		...
		public uint MinPoolSize { 
			get { return _serverPool.MinPoolSize; } 
			set {
		...
		public uint MaxPoolSize {
			get { return _serverPool.MaxPoolSize; } 
			set {
		...
		/// </summary>
		public TimeSpan SocketRecycleAge { get { return _serverPool.SocketRecycleAge; } set { _serverPool.SocketRecycleAge = value; } }

		private uint _compressionThreshold = 1024*128; //128kb
		/// <summary>
		...
		/// </summary>
		public uint CompressionThreshold { get { return _compressionThreshold; } set { _compressionThreshold = value; } }
		...
		private MemcachedClient(string name, string[] hosts) {
			...
			Name = name;
			_serverPool = new ServerPool(hosts);
		}
		...
		private uint hash(string key) {
			CheckKey(key);
			return BitConverter.ToUInt32(new ModifiedFnv132().ComputeHash(Encoding.UTF8.GetBytes(key)), 0);
		}
		...
		private uint hash(uint hashvalue) {
			return BitConverter.ToUInt32(new ModifiedFnv132().ComputeHash(BitConverter.GetBytes(hashvalue)), 0);
		}
		...
		private uint[] hash(string[] keys) {
			var result = new uint[keys.Length];
		...
		private uint[] hash(uint[] hashvalues) {
			var result = new uint[hashvalues.Length];
		...
		private static int GetUnixTime(DateTime datetime) {
			return (int)(datetime.ToUniversalTime() - _epoch).TotalSeconds;
		}
		#endregion
		...
		private CasResult store(string key, bool keyIsChecked, object value, uint hash, int expiry, ulong unique) {
			var result = store("cas", key, keyIsChecked, value, hash, expiry, unique);
		...
		private string store(string command, string key, bool keyIsChecked, object value, uint hash, int expiry, ulong unique) {
			...
			return _serverPool.Execute<string>(hash, "", delegate(PooledSocket socket) {
				SerializedType type;
				byte[] bytes;
				...
				try {
					...
					_logger.Error("Error serializing object for key '" + key + "'.", e);
					return "";
				}
				...
				switch(command) {
					...
					case "add":
					case "replace":
						commandline = command + " " + _keyPrefix + key + " " + (ushort)type + " " + expiry + " " + bytes.Length + "\r\n";
						break;
					case "append":
					case "prepend":
						commandline = command + " " + _keyPrefix + key + " 0 0 " + bytes.Length + "\r\n";
						break;
					case "cas":
						commandline = command + " " + _keyPrefix + key + " " + (ushort)type + " " + expiry + " " + bytes.Length + " " + unique + "\r\n";
						break;
				}
		...
		private object get(string command, string key, bool keyIsChecked, uint hash, out ulong unique) {
			...
			ulong __unique = 0;
			var value = _serverPool.Execute<object>(hash, null, delegate(PooledSocket socket) {
				socket.Write(command + " " + _keyPrefix + key + "\r\n");
				object _value;
				ulong _unique;
		...
		private object[] get(string command, string[] keys, bool keysAreChecked, uint[] hashes, out ulong[] uniques) {
			...
			var dict = new Dictionary<SocketPool, Dictionary<string, List<int>>>();
			for (var i = 0; i < keys.Length; i++) {
				Dictionary<string, List<int>> getsForServer;
				var pool = _serverPool.GetSocketPool(hashes[i]);
			...
			//Get the values
			var returnValues = new object[keys.Length];
			var _uniques = new ulong[keys.Length];
			foreach (var kv in dict) {
				_serverPool.Execute(kv.Key, delegate(PooledSocket socket){
					//Build the get request
					var getRequest = new StringBuilder(command);
					foreach (var key in kv.Value) {
						getRequest.Append(" ");
						getRequest.Append(_keyPrefix);
					...
					while (ReadValue(socket, out gottenObject, out gottenKey, out unique)) {
						foreach(var position in kv.Value[gottenKey]) {
		...
		private bool ReadValue(PooledSocket socket, out object value, out string key, out ulong unique) {
			...
			var parts = response.Split(' '); //Result line from server: "VALUE <key> <flags> <bytes> <cas unique>"
			if (parts[0] == "VALUE") {
				key = parts[1];
				var type = (SerializedType)Enum.Parse(typeof(SerializedType), parts[2]);
				var bytes = new byte[Convert.ToUInt32(parts[3], CultureInfo.InvariantCulture)];
				if (parts.Length > 4) {
				...
				try {
				    ...
				    value = null;
					_logger.Error("Error deserializing object for key '" + key + "' of type " + type + ".", e);
				}
				return true;
			} else {
				key = null;
		...
		private bool delete(string key, bool keyIsChecked, uint hash, int time) {
			...
			return _serverPool.Execute<bool>(hash, false, delegate(PooledSocket socket){
				string commandline;
				if (time == 0) {
					commandline = "delete " + _keyPrefix + key + "\r\n";
				} else {
					commandline = "delete " + _keyPrefix + key + " " + time + "\r\n";
				}
				socket.Write(commandline);
		...
		private ulong?[] getCounter(string[] keys, bool keysAreChecked, uint[] hashes) {
			var results = new ulong?[keys.Length];
			ulong[] uniques;
			var values = get("get", keys, keysAreChecked, hashes, out uniques);
		...
		private ulong? IncrementDecrement(string cmd, string key, bool keyIsChecked, ulong value, uint hash) {
			...
			return _serverPool.Execute<ulong?>(hash, null, delegate(PooledSocket socket) {
				var command = cmd + " " + _keyPrefix + key + " " + value + "\r\n";
		...
		public bool FlushAll(TimeSpan delay, bool staggered) {
			var noerrors = true;
			uint count = 0;
			foreach (var pool in _serverPool.HostList) {
				_serverPool.Execute(pool, delegate(PooledSocket socket) {
					var delaySeconds = (staggered ? (uint)delay.TotalSeconds * count : (uint)delay.TotalSeconds);
		...
		public Dictionary<string, string> Stats(uint hash) { return stats(_serverPool.GetSocketPool(this.hash(hash))); }
		public Dictionary<string, string> StatsByHost(string host) { return stats(_serverPool.GetSocketPool(host)); }
		...
		public Dictionary<string, Dictionary<string, string>> Status() {
			...
			foreach (var pool in _serverPool.HostList) {
				var result = new Dictionary<string, string>();
				if (_serverPool.Execute<bool>(pool, false, delegate { return true; })) {
