...
namespace Core.Web
{
     ...
     public static class WebControlHelper
    {
        ...
        {
            var strJs = (enableNegative) ? "javascript:{if (!/^\\-?\\d+$/.test(this.value)) { this.value='" + iDefault.ToString() + "';}}" : "javascript:{if (!/^\\d+$/.test(this.value)) { this.value='" + iDefault.ToString() + "';}}";
            for (var i = 0; i < tbxes.Length; i++)
            {
        ...
        private static string GetMatchBinding(string strBindObject, ref DataRow dRow)
        {
            ...
            // Escaped: \\{(\\\"|\\')([^\\\"\\']+)(\\\"|\\')\\}
            var strPattern = "\\{(\\\"|\\')([^\\\"\\']+)(\\\"|\\')\\}";
            var mc = Regex.Matches(strBindObject, strPattern, RegexOptions.IgnoreCase);
            var strColumnName = string.Empty;
            var iTotal = mc.Count;
        ...
        {
            var strFmt = objParams[0].ToString();
            var paramValue = new object[objParams.Length - 1];
        ...
        public static string GetOptionData(DataTable dTab, string strValField, string strTxtField, string strValSel)
        {
            ...
            else
            {
                ...
                var optfmt = "<option value=\"{0}\"{2}>{1}</option>";
        ...
        {
            var strSearchSource = separator + chkedValue + separator;
            foreach (ListItem item in cbxList.Items)
            {
